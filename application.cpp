...
Application::~Application()
{
    ...
    vkFreeCommandBuffers(
        logicalDevice,
        uiCommandPool,
        static_cast<uint32_t>(uiCommandBuffers.size()),
...
void Application::createGraphicsPipeline()
{
    ...
    auto vertShaderCode = ShaderLoader::load("shaders/shader.vert.bin");
    auto fragShaderCode = ShaderLoader::load("shaders/shader.frag.bin");
...
void Application::createSyncObjects()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
    {
        ...
        if (vkCreateSemaphore(logicalDevice, &semaphoreCreateInfo, nullptr, &renderFinishedSemaphores[i]) != VK_SUCCESS)
        {
            throw std::runtime_error("Unable to create semaphore!");
        }
...
{
    std::chrono::nanoseconds framePeriod(int(std::round(1.0 / 60.0 * 1e9)));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        auto elapsed_time_ns = std::chrono::nanoseconds(
            int(std::round(std::chrono::duration<double, std::nano>(t_end - t_start).count())));
        auto sleep_time = framePeriod - elapsed_time_ns;
